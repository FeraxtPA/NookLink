    ...
    std::function<void(int)> onSortBooks)
{
    ...
    BuildSettingsPanel(onSelectTheme, getCurrentThemeIndex, getThemeCount, getThemeNameByIndex, onSetLayoutDensity, getLayoutDensity);
...
{
    auto row = CreateButtonRow({ UiMetrics::kPanelWideFieldWidth, UiMetrics::kPanelInputHeight }, 8.0f);
