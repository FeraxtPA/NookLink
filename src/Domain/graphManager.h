...
public:
    ...
    void SetSelectionRectangleWorld(Vector2 worldStart, Vector2 worldEnd);
    void ClearSelectionRectangleWorld();
    void DrawSelectionRectangleWorld() const;

    std::vector<int> GetBookNodeIdsInWorldRect(const Rectangle& worldRect, bool onlyVisible = true) const;
    std::vector<int> GetMultiSelectedBookIds() const;
    bool IsBookMultiSelected(int bookId) const { return m_MultiSelectedBookIds.contains(bookId); }
    void SetMultiSelectedBookIds(const std::vector<int>& bookIds);
    void AddMultiSelectedBookIds(const std::vector<int>& bookIds);
    void ClearMultiSelectedBookIds();
    size_t GetMultiSelectedBookCount() const { return m_MultiSelectedBookIds.size(); }
    bool TranslateMultiSelectedBookNodes(Vector2 delta);
    bool HasSignificantNodeOverlaps(float overlapFactor = 0.82f) const;
private:
    ...
    mutable std::unordered_map<int, size_t> m_NodeIndexByIdCache;
    mutable bool m_NodeIndexByIdCacheDirty = true;

    bool m_SelectionRectActive = false;
    Rectangle m_SelectionRectWorld{ 0, 0, 0, 0 };
    std::unordered_set<int> m_MultiSelectedBookIds;
