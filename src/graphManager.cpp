...
void GraphManager::releaseDraggedNode() {
    Node* dragged = getDraggedNode(); // Použije naši novou metodu
    if (dragged) {
...
void GraphManager::applyLoadedPositions(const std::unordered_map<int, NodePosition>& loadedPos) {
    if (loadedPos.empty()) return;
...
void GraphManager::updatePhysics(float dt) {
    ...
    m_IsPhysicsActive = m_GraphLayout.updatePhysics(m_Nodes, bookToGenreMap, centerPos, dt, draggedNode);
}
...
float GraphManager::calculateCircleRadius(int nodeCount, float minRadius) const
{
    ...
    return std::max(minRadius, (nodeCount * 2.0f * GraphConfig::BaseNodeRadius) / (2.0f * PI));
}
    ...
    const std::unordered_map<std::string, Vector2>& oldPositions)
{
    ...
    float genreRadius = GraphConfig::BaseNodeRadius * GraphConfig::GenreRadiusMultiplier;
...
void GraphManager::placeBookNodes(const std::unordered_map<int, Vector2>& oldPositions) {
    ...
    for (const auto& book : m_BookManager.getBooks()) {
        ...
        Vector2 pos = {
            avgPos.x + GraphConfig::BookSpawnOffset * cos(angle),
...
void GraphManager::removeNodeById(int id) {
    auto removeSingleNode = [&](int targetId) {
        ...
        // OPRAVA: Kontrola taženého uzlu pøes ID
