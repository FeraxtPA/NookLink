...
constexpr const char* kSettingsToolbarGlyph = "⚙";
}
...
void UIManager::BuildFilterPanel(const std::function<void(Status)>& onToggleStatus, const std::function<void(int)>& onSortBooks)
{
    ...
    m_SearchFilterPanel->isVisible = false;
    ...
    auto helpLabel = std::make_shared<Label>(
        ...
        "Query help: duna | r>4 | g:fantasy | s:reading | fr:month",
        18,
    ...
    auto quickFantasyBtn = std::make_shared<Button>(
        Anchor::TopLeft,
        Vector2{ 0.0f, 0.0f },
        Vector2{ 0.0f, UiMetrics::kPanelButtonRowHeight },
        "Fantasy",
        [applyPreset]() { applyPreset("g:fantasy", 0.0f, "fantasy", 0); }
    );
    quickRow2->AddChild(quickReadingBtn, { 0.0f, UiMetrics::kPanelButtonRowHeight }, 1.0f);
    quickRow2->AddChild(quickFantasyBtn, { 0.0f, UiMetrics::kPanelButtonRowHeight }, 1.0f);
    ...
    const std::function<void()>& onToggleLayout)
{
    ...
    m_ToggleFiltersBtn = makeToolbarButton(kFilterToolbarGlyph,
        [this]() {
    ...
    m_OpenStatsBtn = makeToolbarButton("Stats",
        [this]() {
            if (m_AnalyticsPanel) {
                m_AnalyticsPanel->isVisible = !m_AnalyticsPanel->isVisible;
            }
        },
    ...
    m_ToolbarLayout->AddChild(makeToolbarButton("Add Book", [this]() { m_AddPanel->isVisible = true; }, UiMetrics::kToolbarAddBookWidth), { UiMetrics::kToolbarAddBookWidth, toolbarRowHeight });
